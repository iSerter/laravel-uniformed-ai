...
it('injects query param key for google and skips auth header', function () {
    ...
    $client->get('v1beta/test');
    Http::assertSent(function ($req) {
        return !$req->hasHeader('Authorization') && str_contains($req->url(), 'key=g-key');
    });
});
